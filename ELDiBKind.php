...
if ( isset($_GET['SetELDiBKind']) ){
    ...
    $Inhalt = $Inhalt . '  <h1>ELDiB Bewertungsbogen Kind</h1><button type="button" onclick="HideELDiBKind();" class="ButtonBack">zurück</button>';
    ...
    $Inhalt = $Inhalt . '<div>';
    if($TableStatus == 0){
      $Inhalt = $Inhalt .'<button type="button" class="CreateNewTableELDiBKEL" onclick="CreateNewTableELDiBKind();">neuen Bogen erstellen</button>';
    }
    $Inhalt = $Inhalt . '</div>';
    ...
    $Inhalt = $Inhalt .'</div>';
    $Inhalt = $Inhalt .'<div id="ELDiBKindcontainer" class="tableFixHead">';
    $Inhalt = $Inhalt .'<table>';
    $Inhalt = $Inhalt .'<thead>';    
    ...
    if ($TableStatus >= 1){
      $Inhalt = $Inhalt .'<td><button type="button" onclick="SaveTableELDiBKind();">Bogen speichern</button></td>';
    }
  ...
  for($BereichID = 1; $BereichID <=4; $BereichID++){
    ...
    while ($zeile = mysqli_fetch_assoc( $db_erg))
      ...
      $Inhalt = $Inhalt .'<td class="ZielBeschreibung TabContent'.$BereichID.'" ondblclick="readText(this)"> '.MyStringHTML($zeile['ZieleBeschreibung']).'</td>';
      $Inhalt = $Inhalt .'<td class="ZielNummer TabContent'.$BereichID.'" ondblclick="readText(this)">'.$zeile['ZieleNummer'].' '.MyStringHTML($zeile['ZieleStichwort']).'</td>';
      ...
      if ( ! $db_ergdata )
        ...
        $Inhalt = $Inhalt .'<tr>';
        if($TableStatus == 1){  
        $Inhalt = $Inhalt . '<td><div class="StichwörterDesc">Stichwörter <input type="text" onchange="ChangedKeywordELDiBKind('.$zeile['id'].','.$idData.',this.value);" class="Stichwörter" value="'.$keyword.'"></input></div></td>';
        }
        else{
